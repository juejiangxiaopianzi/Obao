...
| skill | 适合谁 | 怎么干 |
|---|---|---|
| 🟦 **obao-aily-review**（飞书 aily 智能体首选） | 艾丽这类**飞书里的智能体** | 用 aily **内置的飞书云文档/评论/任务工具**，零 lark-cli、不新建应用、不用分享文档。复制周报副本 → 标评论 → 你在副本上改/删/回复纠正 → 「可以推送了」→ 发回原周报 + 自学 + 删副本。 |
| 🟦 **obao-feishu-loop**（飞书外的 agent） | Claude Code / Cursor / Codex 等**飞书外面**的 agent | 同一套闭环，但用 **lark-cli** 调飞书（需要你本机已登录 lark-cli）。 |
| ⬜ **obao-review**（兜底） | 没飞书的人 | 生成一份**本地 HTML 审阅页**，自己看、自己复制评论。 |

> 三条线是**同一套方法论**（审 → 副本承载 → 你评论纠正 → 推回原周报 → 案例库自学），只是"怎么碰飞书"不同：
> 飞书里的智能体用**自己的原生工具**、飞书外的 agent 用 **lark-cli**、没飞书的退回 **HTML**。
> 为什么不用本地 HTML 当主力？因为它是**死文件**——你改了 agent 看不见、回路不通；飞书文档是**双向**的，纠正才能真反哺。
