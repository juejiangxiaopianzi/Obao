...
1. **目标是硬门槛**：每份周报先匹配到可信的 OKR/OGSM（自动从周报内嵌/历史存档/本次对话/飞书 OKR 拿；拿不到就停下问你；只有你明确说"不用"才无目标降级）。**绝不从周报内容反推一套目标冒充正式 OKR**——那样审出来的"对齐/缺口"是假的。
2. **评论是块级锚点，不是真划词高亮**：飞书开放接口创建评论只支持 `anchor.block_id`（块级），实测无字符范围参数。所以评论自带原文定位、**是否成功以你的前端为准**（评论卡可见 + 定位得到），`comment_id` 返回不算数。做不到真划词 Obao 会老实说，不伪造。
...
完整方法论看 [`skills/obao-review/assets/prompt.md`](./skills/obao-review/assets/prompt.md)（9 步加工流程 + A-K 红线）。
...
### v0.7.0（2026-06）
- **目标是硬门槛**：每份周报先匹配可信目标（OKR/OGSM）。自动获取链：周报内嵌 → 历史存档 `profile.md` → 本次对话给的链接/文本 → 飞书 OKR API；拿到不打扰你；**拿不到就停下来只问你要目标**（飞书链接 / OKR 产品页 / 离线文件 / 纯文本都收）；**只有你明确拒绝才无目标降级**。⛔ **绝不从周报反推目标冒充正式 OKR**。
- **以目标为骨架**：所有 KR 都呈现；某条 KR 周报没提**不再一刀切判"无进展"**，先判 ✅已完成 / ⏸暂停 / ⚠️真停滞。
- **游离项逐项审**：统一思考链（自身管理价值 + 与目标关系）。原则——**"是否对应 KR"只决定事项放在哪里；"事项是否值得管理关注"才决定是否需要评论**，不许因游离/已完成/不属于 KR 就跳过。
- **诚实评论能力**：实测确认飞书开放接口只支持**块级锚点**（`anchor.block_id`），不是真划词高亮；评论自带原文定位、`comment_id` ≠ 成功、**验收以用户前端为准**、做不到真划词就老实说不伪造、不擅自把大量评论塞承载页正文。新增红线 K，修正旧文里"飞书划词已锚定具体句"的错误前提。
