...
| skill | 适合谁 | 怎么交付 |
|---|---|---|
| 🟦 **obao-feishu-loop**（主推） | 用飞书的人 | **复制一份周报副本**当承载页 → 把追问标成划词评论 → 你在副本上**改/删/回复**就是纠正 → 你说「可以推送了」→ 它把认可的发回**原周报**、按你的纠正**自学**、再删副本。一个活的闭环。 |
| ⬜ **obao-review**（兜底） | 没飞书的人 | 生成一份**本地 HTML 审阅页**，你自己看、自己复制评论。 |

> 为什么主推飞书闭环？因为本地 HTML 是**死文件**——你在上面改了，agent 看不见、回路不通。飞书副本是**双向**的：agent 写得上、你改得动、agent 读得回，纠正能真的反哺、越用越像你。
