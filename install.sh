...
# Workpad install — copy skill into Claude Code skill folder
SCRIPT_DIR="$( cd "$( dirname "${BASH_SOURCE[0]}" )" && pwd )"
SKILL_SRC="$SCRIPT_DIR/skill"
SKILL_DST="$HOME/.claude/skills/workpad-review"
...
if [[ -d "$SKILL_DST" ]]; then
  echo "Existing workpad-review skill found at $SKILL_DST"
  echo "Backing up to ${SKILL_DST}.bak.$(date +%s)"
  mv "$SKILL_DST" "${SKILL_DST}.bak.$(date +%s)"
fi

cp -R "$SKILL_SRC" "$SKILL_DST"
echo "✓ Installed workpad-review skill to $SKILL_DST"
echo ""
echo "Next steps:"
...
echo "  4. Paste those into Claude and let it run workpad-review"
echo ""
echo "✓ Done."
